...
\item \JWnamedlink{http://code.google.com/p/protobuf-c/}{protobuf-c} for parsing
Protocol Buffer files in generating code in C
...
, a R library for regression subset selection (to minimize the set of available
...
\item \JWnamedlink{http://kernel.org}{Linux Kernel 3.0}
...
As one could understand until here, there are many simple tools. To compose
...
also mostly avoid the excluded units because their results would then have a bad
consequences on the final model.

In this work all benchmarks of the \JWTLspec{} suite, the memory bandwith
benchmark \JWTLstream{} a benchmark provokating many branch mispredictions
and an ALU stress benchmark have been used.
...
event is triggered) one pseudo-event per core modelling the time this specific
core is enabled has been added. These events fall in the group of the
\emph{global events} (see chapter \ref{sec:model-properties}). Because the
setting if one core is enabled or not can only be changed when (re)booting the
machine each of these pseudo-events is either $0$ -- if the core is disabled --
or equal to the entire running time of a benchmark -- if the core is enabled.
...
distributed in disjoint groups of eight events -- the maximal number of
...
considered as one run recording all the events available.  Since the electrical
...
differ much between the runs, the relative standard deviation was below 4\% for
the worst case.
...
After the event selection process (see chapter \ref{sec:finding-useful-subset})
energetical weights for the (pseudo-)event had to be found. To find these, the
benchmarks were first rerun again only on one core but recording all of
the final events in one shot. Thereafter all reasonable permutations for two and
three cores and 500 random permutations for all four cores have been generated.
This new huge set of benchmarks has then been executed, always rebooting and
reconfiguring the machine to the correct number of enabled cores. Figure
\ref{fig:run-benchmarks} illustrates the process. The pinning of the benchmarks
to their target core has been done using the \texttt{taskset} utility.

In this section the final energy model is presented:

\JWtodo{Tabelle vierte Spalte: max-betrag(P)}
